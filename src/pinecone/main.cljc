(ns pinecone.main
  "Kotodama WASM entrypoint for the Pinecone clean-room actor (L4) — Clojure port.
  ...
  facts (`pinecone.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pinecone")
(def tier "L4")
...
(def entity-specs
  [{:entity "Index"      :plural "indexes"     :id-prefix "pinecone_ind"
    :fields [:name :dimension :metric :host :state :deletionProtection :vectorType :cloud :ready] :required [:name :dimension]
    :coerce {:dimension :int :ready :bool}  :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "pinecone_col"
    :fields [:name :dimension :status :vectorCount :size :environment] :required [:name :dimension]
    :coerce {:dimension :int :vectorCount :int :size :int}  :refs {}}
   {:entity "Vector"     :plural "vectors"     :id-prefix "pinecone_vec"
    :fields [:namespace]                    :required [:namespace]
    :coerce {}                       :refs {}}
   {:entity "Namespace"  :plural "namespaces"  :id-prefix "pinecone_nam"
    :fields [:name :recordCount]              :required [:name :recordCount]
...
(defn emit-facts
  "EAVT facts for one record: {\"pinecone.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pinecone-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pinecone-compat/ready)
